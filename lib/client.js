...
var ModeShapeClient = function(options){
    ...
    //Initialize base url
    this.baseUrl = url.format({ protocol: this.options.protocol, hostname: this.options.hostname, port: this.options.port }) + "/" + this.options.context;
};
...
ModeShapeClient.prototype.getAvailableRepositories = function(callback){
    ...
    this.executeRequest(internals.GET_METHOD, this.getEndPointUrl(''), function(err, res) {
